...
function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'dodge') then 
		npcHandler:say("Você quer trocar {5 event coins} por {1 dodge}?", cid)
		npcHandler.topic[cid] = 1
	end
		
	if msgcontains(msg, 'critical') then 
		npcHandler:say("Você quer trocar {5 event coins} por {1 critical}?", cid)
		npcHandler.topic[cid] = 2
	end
		
	if msgcontains(msg, 'event ring') then 
		npcHandler:say("Você quer trocar {1 event coin} por {1 event ring}?", cid)
		npcHandler.topic[cid] = 3
	end
	
	if msgcontains(msg, 'might ring') then 
		npcHandler:say("Você quer trocar {1 event coin} por {1 might ring}?", cid)
		npcHandler.topic[cid] = 4
	end
	
	if msgcontains(msg, 'stone skin amulet') then 
		npcHandler:say("Você quer trocar {1 event coin} por {1 stone skin amulet}?", cid)
		npcHandler.topic[cid] = 5
	end
	
	if msgcontains(msg, 'stamina refiller') then 
		npcHandler:say("Você quer trocar {5 event coins} por {1 stamina refiller}?", cid)
		npcHandler.topic[cid] = 6
	end
	
	if msgcontains(msg, 'event backpack') then 
		npcHandler:say("Você quer trocar {35 event coins} por {1 event backpack}?", cid)
		npcHandler.topic[cid] = 7
	end
	
	if msgcontains(msg, 'bronze vip days') then 
		npcHandler:say("Você quer trocar {10 event coins} por {1 bronze vip days}?", cid)
...
-----------------------------------------------------------------------------------
	...
	if msgcontains(msg, "yes") or msgcontains(msg, "sim") then	
		if npcHandler.topic[cid] == 1 then -- dodge
			if getPlayerItemCount(cid, 12686) >= 5 then
				if doPlayerRemoveItem(cid, 12686, 5) then
		...
		elseif npcHandler.topic[cid] == 3 then -- event ring
			if getPlayerItemCount(cid, 12686) >= 1 then
		...
		elseif npcHandler.topic[cid] == 7 then -- event backpack
			if getPlayerItemCount(cid, 12686) >= 35 then
				if doPlayerRemoveItem(cid, 12686, 35) then
		...
		elseif npcHandler.topic[cid] == 8 then -- bronze vip days
			if getPlayerItemCount(cid, 12686) >= 10 then
				if doPlayerRemoveItem(cid, 12686, 10) then
