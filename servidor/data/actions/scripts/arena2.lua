...
local arena_from = {x=2039,y=2023,z=9}
local arena_to = {x=2045,y=2030,z=9}
...
function onUse(cid, item, frompos, item2, topos)
    local player1 = getTopCreature({x=2040,y=2020,z=8}).uid
	local player2 = getTopCreature({x=2041,y=2020,z=8}).uid
	
	if item.itemid == 1945 then
		if isPlayer(player1) and isPlayer(player2) then
			doTransformItem(item.uid, item.itemid + 1)
			addEvent(changeBack, mathtime(timelever) * 1000, {x=2040,y=2021,z=8})
			setGlobalStorageValue(cid, 30078, os.time()+5*60)
			doRemoveMonstersInArea(arena_from, arena_to)
			doTeleportThing(player1, {x=2042,y=2023,z=9})
			doTeleportThing(player2, {x=2042,y=2030,z=9})
			doPlayerSendTextMessage(player1, MESSAGE_STATUS_WARNING, "Fight!")
			doPlayerSendTextMessage(player2, MESSAGE_STATUS_WARNING, "Fight!")
			doCreatureSay(cid, "O duelo começou!", TALKTYPE_ORANGE_1, false, 0, {x=2042,y=2026,z=9})
			doCreatureSay(cid, "Vocês tem 4 minutos.", TALKTYPE_ORANGE_1, false, 0, {x=2042,y=2027,z=9})
	...
	elseif item.itemid == 1946 then
		return doCreatureSay(cid, "Aguarde ".. convertTime(getGlobalStorageValue(cid, 30078) - os.time()) ..".", TALKTYPE_ORANGE_1, false, 0, {x=2040, y=2021, z=8})
	end
	return true
end