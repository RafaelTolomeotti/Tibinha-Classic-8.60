...
function onThink(interval, lastExecution)
	local result_plr = db.getResult("SELECT * FROM z_ots_guildcomunication")
				...
				if(action == 'give_outfit') then
					if outfit_name ~= "" and maleOutfits[outfit_name] and femaleOutfits[outfit_name] then
						...
						if not canPlayerWearOutfit(cid, add_outfit, 3) then
							...
							db.query("DELETE FROM `z_ots_guildcomunication` WHERE `id` = " .. id .. ";")
							db.query("UPDATE `accounts` SET `guild_points` = `guild_points` + " .. points .. " WHERE `id` = " .. getPlayerAccountId(cid) .. ";")
						end
					end
				end
